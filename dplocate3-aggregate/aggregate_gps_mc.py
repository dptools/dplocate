...
def main():
    ...
    for study in studies:
        ...
        for subject in subjects:
            ...
            data_path = args.input_dir if args.input_dir else os.path.join(subject_path, args.data_type, 'processed')
...
def verify_subject(subject, path, consents):
    ...
    if subject.endswith('.csv'):
        ...
        return False
    logger.info('Processing {S} {T}'.format(S=os.path.isdir(path), T=path))
    if not os.path.isdir(path):
        logger.error('Path {P} does not in exist.'.format(P=path))
        return False
