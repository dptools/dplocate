...
def parse_args():
    argparser = ap.ArgumentParser('aggregate_gps_mc Pipeline for Beiwe GPS')
...
def main(args):
    ...
    read_dir = os.path.join(read_dir,'gps_dash2')
    ...
    # logger output
    fh = logging.FileHandler(os.path.join(output_log,  'aggregate_gps_mc.log'))
    logger.addHandler(fh)

    # run MATLAB
    run_matlab(read_dir, output_dir, args.date_from, 
        args.study, args.subject, args.matlab_dir)

# Run MATLAB
def run_matlab(read_dir, output_dir, date_from, study, subject, matlab_dir):
    try:
        matlab_path = "addpath('{matlab_dir}');".format(matlab_dir=matlab_dir)
        sub_cmd = "aggregate_gps_mc('{ST}','{SB}','{C}','{I}','{O}','{M}')".format(ST=study,
            SB=subject,C=date_from,
            I=read_dir, O=output_dir, M=matlab_dir)
...
def clean_output_dir(output_dir):
    ...
    file_path = os.path.join(output_dir, 'gps_dash2/daily_all.mat.lock')
    if os.path.exists(file_path):
        logger.warn('Removing file %s' % file_path)
        os.remove(file_path)
