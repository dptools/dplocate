...
def parse_args():
    argparser = ap.ArgumentParser('phone_gps_mc Pipeline for Phone GPS')
...
def main(args):
    ...
    # logger output
    fh = logging.FileHandler(os.path.join(output_dir,args.study,args.subject,'phone/processed/phone_gps_mc.log'))
...
def run_matlab(input_dir, output_dir, extension, matlab_dir, date_from, subject, study):
    try:
        ...
        sub_cmd = "phone_gps_mc('{INPUT_DIR}','{OUTPUT_DIR}','{EXTENSION}','{matlab_dir}','{date_from}','{subject}','{study}')".format(OUTPUT_DIR=output_dir, INPUT_DIR=input_dir, EXTENSION=extension, matlab_dir=matlab_dir,date_from=date_from,subject=subject,study=study)
